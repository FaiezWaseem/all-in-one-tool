import {
    getDatabase, ref, get, set, push, child,
    onValue, onChildAdded, onChildChanged,
    onChildRemoved , update , remove
...
class firebase {
    ...
    public set(path: string, data: object, callback: Function = (e) => console.log(e)) {
        set(ref(this.db, path), data).then(() => callback('saved'))
            .catch((err) => callback(err))
    }
    public push(path: string, data: object, callback: Function = (e) => console.log(e)) {
        push(ref(this.db, path), data).then(() => callback('saved'))
            .catch((err) => callback(err))
    }
    public update(path: string, data: object, callback: Function = (e) => console.log(e)) {
        update(ref(this.db, path), data).then(() => callback('updated'))
            .catch((err) => callback(err))
    }
    public remove(path: string ,callback: Function = (e) => console.log(e)) {
        remove(ref(this.db, path)).then(() => callback('deleted'))
            .catch((err) => callback(err))
    }
