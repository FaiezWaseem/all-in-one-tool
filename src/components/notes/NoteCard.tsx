...
import { useDisclosure , Button , Modal  , ModalContent , ModalOverlay ,FormLabel , ModalHeader, ModalFooter,
    FormControl , ModalCloseButton , ModalBody , Input , Textarea
} from '@chakra-ui/react';
import firebase from '@/backend/firebase'



export default function NoteCard({item , ip} ) {
   ...
   function onUpdate(){
    ...
    return (
        <div onClick={onOpen} ref={finalRef}>
            <div className={styles.note_common}  style={{ width: 260, height: 260 , backgroundColor : bgColor }}>
                <div style={{
                    "position": "absolute",
                    "zIndex": "99",
                    "top": "-30px",
                ...
                dangerouslySetInnerHTML={{ __html :noteContent.length === 0 ? '<p>No content start writing ..<p>' : parseSyntax(noteContent)}}
                >
          ...
          <ModalContent>
            ...
            <ModalBody pb={6}>
              <FormControl>
                ...
                <Input ref={initialRef} placeholder='add title here...' value={title} onChange={(e) =>{
              ...
              <FormControl mt={4}>
                ...
                <Textarea placeholder='add content here' value={content} onChange={(e)=>{
...
function parseSyntax(data){
  ...
  data.forEach(item => {
      ...
      if(item[0] == '#'){
          ...
          let tag = `h${hCount}`;
          output += `<${tag} style="line-height:3ch">${item.slice(hCount, item.length)}</${tag}>`
      } 
